...
import { handleChangeVoyageActivityAuxEngnInput3 } from "../../../_redux/actions/VoyageActivityAuxEngnAction3";
...
const AuxEngine3 = (props) => {
  ...
  const voyageActivityAuxEngnInput = useSelector(
    (state) => state.voyageAux3.voyageActivityAuxEngnInput
  );
  const loading = useSelector((state) => state.voyageAux3.isLoading);
  const voyageActivityCreateInput = useSelector(
    (state) => state.voyageActivityInfo.voyageActivityCreateInput
  );

  const handleChangeTextInput = (name, value) => {
    dispatch(handleChangeVoyageActivityAuxEngnInput3(name, value));
  };
  ...
  return (
    ...
    >
      <div>
        <p className="text-uppercase text-bold mt-3">Aux.Engine 3</p>
      </div>
        ...
        <div className="col-lg-2">
          <Form.Group>
            <Form.Control
              ...
              onChange={(e) => handleChangeTextInput("dceTCRPM", e.target.value)}
            />
          </Form.Group>
        </div>
      </div>
