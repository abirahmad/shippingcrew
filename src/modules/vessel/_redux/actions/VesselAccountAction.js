...
export const GetVesselAccountList = (intVesselId) => async (dispatch) => {
    let data = {};
    const headers = {
    ...
    axios
        .get(
            `http://iapps.akij.net/asll/public/api/v1/asllhr/getVesselAccountTransaction?intVesselId=${intVesselId}`,
            { headers: headers }
        )
        .then((res) => {
            console.log('ResponseAccountVessel',res);
            let data = res.data;
            dispatch({ type: Types.GET_VESSEL_ACCOUNT_LIST, payload: data });
        });
