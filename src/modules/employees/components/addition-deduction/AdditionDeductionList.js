...
const AdditionDeductionList = withRouter(({ history, props }) => {
  ...
  useEffect(() => {
    ...
    dispatch(GetAdditionDeductionListByEmployee());
  ...
  const searchEmployee = (event) => {
    ...
    const searched_employees = employeeInfoList.filter(function(item) {
      let strName = item.intID + " " + item.strEmail;
  ...
  const addMultipleValue = (e) => {
    ...
    //   if (addition.images === "") {
    //     message += "Attachment Can't be Blank in Cash Advance";
    //   }
    // }
    ...
    let additionDataObj = {
      ...
      intVesselItemId:
        additionData.AdditionDeductionType.label == "Bond Issue"
      ...
      strVesselItemName:
        additionData.AdditionDeductionType.label == "Bond Issue"
          ...
          : "",
      intMonthId: additionData.monthData.label,
